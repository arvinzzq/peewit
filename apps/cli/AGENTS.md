...
CLI-visible behavior, session listing/resume, workspace prompt and read-only long-term/daily memory handoff, interactive input loops, built-in file, shell, and web page tool registration, approval prompts, durable message and trace handoff, short-term memory handoff, slash commands, trace rendering, config redaction and memory policy display, and missing API key handling need tests in `src/index.test.ts`. Use injected fake HTTP for configured-provider tests. Do not require a real model provider or API key in CLI unit tests.
