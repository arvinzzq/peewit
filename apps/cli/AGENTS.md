...
Keep this package focused on terminal adaptation. It may compose runtime, config, workspace-aware and memory-policy-aware context assembly including daily memory, and durable session/message/trace dependencies for CLI commands, but agent behavior belongs in `packages/core`, prompt/context work belongs in `packages/context`, session persistence belongs in `packages/sessions`, and provider normalization belongs in `packages/models`.
...
CLI-visible behavior, session listing/resume, workspace prompt and read-only long-term/daily memory handoff, interactive input loops, durable message and trace handoff, short-term memory handoff, slash commands, trace rendering, config redaction and memory policy display, and missing API key handling need tests in `src/index.test.ts`. Use injected fake HTTP for configured-provider tests. Do not require a real model provider or API key in CLI unit tests.
