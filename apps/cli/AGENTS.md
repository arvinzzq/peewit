...
Keep this package focused on terminal adaptation. It may compose runtime, config, workspace-aware context assembly, and durable session/message/trace dependencies for CLI commands, but agent behavior belongs in `packages/core`, prompt/context work belongs in `packages/context`, session persistence belongs in `packages/sessions`, and provider normalization belongs in `packages/models`.
