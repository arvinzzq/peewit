...
CLI-visible behavior, interactive input loops, slash commands, trace rendering, and config redaction need tests in `src/index.test.ts`. Fake-provider smoke paths are allowed. Do not require a real model provider or API key in CLI unit tests.
