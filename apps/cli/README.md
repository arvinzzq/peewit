...
| `src/index.ts` | CLI adapter | Parses commands, lists and resumes stored sessions, runs configured and fake interactive chat loops, persists JSONL messages/traces, and renders `/help`, `/trace`, `/config`, and `/exit`. |
| `src/index.test.ts` | CLI tests | Protects help, version, session listing/resume, configured chat, durable message/trace handoff, short-term memory handoff, fake-provider chat, compact trace rendering, `/trace`, `/config`, missing API key handling, and unknown-command behavior. |
