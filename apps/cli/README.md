...
| `src/index.test.ts` | CLI tests | Protects help, version, session listing/resume, workspace prompt and long-term/daily memory handoff, configured chat, durable message/trace handoff, short-term memory handoff, fake-provider chat, compact trace rendering, `/trace`, `/config` memory policy output, missing API key handling, and unknown-command behavior. |
