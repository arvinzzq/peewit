...
| `src/server.ts` | Hono API server | POST /api/sessions（创建或恢复）、GET /api/sessions（从 JsonlSessionStore 列出）、GET /api/sessions/:id（单个 session metadata）、GET /api/sessions/:id/messages（消息）、POST /api/sessions/:id/turns（SSE 流）、POST /api/sessions/:id/approvals（解析审批）、GET /api/gateway/sessions（从模块级 SessionGateway 列出活跃 sessions）；创建时在 SessionGateway 中注册 sessions，每次 turn 后更新 lastActivityAt；生产环境下提供静态客户端文件。 |
