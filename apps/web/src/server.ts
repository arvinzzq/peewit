...
async function createWebSession(config: EffectiveConfig): Promise<WebSession> {
  const id = `session_${crypto.randomUUID()}`;
  const approvalResolver = new WebApprovalResolver();
  const store = new InMemorySessionStore({ createSessionId: () => id });
  ...
  const session: WebSession = { id, runtime, store, traceStore, approvalResolver };
...
app.post("/api/sessions", async (c) => {
  ...
  const session = await createWebSession(config);
  return c.json({ sessionId: session.id });
});

// GET /api/sessions — list sessions
app.get("/api/sessions", (c) => {
  const list = Array.from(sessions.keys()).map((id) => ({ id }));
...
app.get("/api/sessions/:id/messages", async (c) => {
  ...
  const messages = await session.store.listMessages(id);
...
app.post("/api/sessions/:id/turns", async (c) => {
  ...
  const recentRaw = await session.store.listMessages(id, { limit: 12 });
  ...
  return streamSSE(c, async (stream) => {
    ...
    for await (const event of session.runtime.runTurn({ sessionId: id, recentMessages, message })) {
      ...
      await session.store.appendTraceEvent({ sessionId: id, event });
    ...
    await session.store.appendMessage({ sessionId: id, role: "user", content: message });
    if (assistantText !== "") {
      await session.store.appendMessage({ sessionId: id, role: "assistant", content: assistantText });
    }
  });
});
