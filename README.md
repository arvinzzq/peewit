...
[![Tests](https://img.shields.io/badge/tests-255%20passing-22c55e)](#development)
...
- **Sub-agent spawning** — `spawn_subagent` tool for decomposing complex tasks
...
- **Task history** — `arvinclaw tasks` lists run records

### Model Providers
- **OpenAI-compatible** — any API following OpenAI chat completions (OpenAI, OpenRouter, Ollama, etc.)
- **Anthropic** — native SDK with prompt caching and streaming
...
| Skill body on-demand loading | 🚧 Planned |
| `memory_search` / `memory_get` tools | 🚧 Planned |
| Cron daemon | 🚧 Planned |
| Hooks system | 🚧 Planned |
| TaskFlow (persistent task graph) | 🚧 Planned |

See [OpenClaw Alignment Plan](./docs/plans/openclaw-alignment.md) for the full roadmap of remaining gaps.
