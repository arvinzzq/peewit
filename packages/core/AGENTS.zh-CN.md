...
保持这个 package 专注于 runtime coordination 和共享 runtime contracts。它可以编排注入依赖，但不应该读取环境变量、渲染 CLI output 或直接调用厂商 API。
...
Runtime event order、failure events 和 dependency injection behavior 需要单元测试。使用 fake providers 和 fake assemblers；不要使用真实 API keys。
