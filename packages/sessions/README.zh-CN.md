...
`@arvinclaw/sessions` 负责**会话持久化边界**：以会话为范围存储对话消息和运行时 trace 事件，将持久化关注点与运行时编排、权限逻辑和 UI 渲染完全分离。
...
`SessionTraceEventRecord<TEvent>` 是对任意事件类型的泛型包装（通常是 `@arvinclaw/core` 的 `RuntimeEvent`）。
