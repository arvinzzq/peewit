...
| `src/index.ts` | Session store | 导出 session/message contracts，以及带 session listing 的 in-memory 和 JSONL session storage。 |
| `src/index.test.ts` | Session tests | 保护 session creation、session listing order、message ordering、recent-message queries、defensive copies、JSONL replay 和 unsafe session ID rejection。 |
