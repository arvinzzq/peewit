...
| `src/index.ts` | Session store | Exports session/message contracts plus in-memory and JSONL session storage. |
| `src/index.test.ts` | Session tests | Protects session creation, message ordering, recent-message queries, defensive copies, JSONL replay, and unsafe session ID rejection. |
