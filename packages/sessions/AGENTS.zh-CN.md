...
把 durable session 和 trace persistence 保持在这里。Runtime code 应依赖 session interface，而不是了解 storage details。
...
Session logic 将需要 create、append、load、ordering、write safety 和未来 replay behavior 的测试。
