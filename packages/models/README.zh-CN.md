...
`@vole/models` 定义 **Provider 无关的模型契约**，并在其背后实现具体的 Provider。所有需要调用 LLM 的包都只依赖 `ModelProvider` 接口——绝不直接依赖 vendor SDK。
...
| `"tool_calls"` | 模型请求工具调用 | `calls: ModelToolCall[]`，可选 `text?: string`（模型在工具调用同一响应中生成的文本），可选 `usage` |
...
`generateStream` yield 的流事件：`token_delta`（文本 token 到达）、`message_done`（流以文本完成）、`tool_calls`（流以工具调用完成，同时携带 `text?: string`——模型在工具调用同一响应中生成的文本）、`error`（流失败）。
