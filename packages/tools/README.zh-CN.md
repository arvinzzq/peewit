...
`@arvinclaw/tools` 负责**工具能力边界**：定义工具的存在形式、输入格式及执行方式。它不决定工具是否被允许运行——这是 `@arvinclaw/permissions` 的专属职责。
...
`ExecutableTool.risk` 是描述工具动作内在风险的元数据，实际的允许/询问/拒绝决定由 `@arvinclaw/permissions` 的 `PermissionPolicy` 做出，结合风险级别和当前自主模式。这使工具、权限策略、运行时三者相互独立，可分别测试和替换。
