...
| `src/index.ts` | Memory tools + hybrid retrieval + DREAMS workflow + FTS5 index | Exports `memoryPackageName`, `EmbeddingProvider`, `EmbeddingProviderName`, `FakeEmbeddingProvider`, `MemorySearchToolOptions`, `createMemorySearchTool`, `createMemoryGetTool`, `createAppendDailyMemoryTool`, DREAMS.md primitives (`DreamEntry`, `DreamEntryStatus`, `parseDreamsFile`, `serializeDreamsFile`, `readDreamsFile`, `applyDreamDecision`), and the Phase 14b Step 5 FTS5 index: `SQLITE_MEMORY_INDEX_SCHEMA_SQL`, `SqliteMemoryIndex`, `SqliteMemoryIndexOptions`, `MemoryIndexSearchResult`, `openWorkspaceMemoryIndex`. |
