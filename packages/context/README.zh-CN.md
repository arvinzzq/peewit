...
`@vole/context` 负责 **Provider 无关的 context 组装**。它的工作是在任何 provider 特定格式化发生之前，决定模型看到什么内容。它接受原始输入（系统指令、运行时元数据、工具摘要、技能索引、权限指导、对话历史、用户消息），并生成可供任何 `ModelProvider` 使用的 `ModelInput`。
