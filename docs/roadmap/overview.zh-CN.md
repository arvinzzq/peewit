...
| Phase 13 | 部分 | 记忆与提示增强 | 基础：@vole/memory package、inline 指令解析器。混合搜索 + DREAMS + flush + prompt sections 在 13b。 | @vole/memory 拆分、parseInlineDirectives、EmbeddingProvider 接口、MemoryFlushOptions |
| Phase 14 | 部分 | SQLite 存储统一升级 | SqliteSessionStore + SqliteTaskFlowStore 已交付；记忆索引 + 迁移工具在 14b | SQLite store、FTS5 记忆索引、迁移工具 |
...
状态：部分。计划文档：[phase-13-memory-and-prompt-enhancement.zh-CN.md](../plans/phase-13-memory-and-prompt-enhancement.zh-CN.md)。
...
本次新增架构：新 `packages/memory`，含三个记忆工具与预留的 `EmbeddingProvider` 接口；`CompactionOptions` 上的 `MemoryFlushOptions`（数据形态就绪；runtime hook 推迟）；`@vole/context` 的 `parseInlineDirectives`，在 intake 阶段剥除 `/think:<level>`、`/stop`、`/compact` 并应用；CLI 新增 `vole compact` 说明命令。

推迟到 Phase 13b（在 Phase 14 与 15 之后）：真正的 OpenAI + Voyage embedding 适配 + 带 reciprocal rank fusion 的混合 `memory_search`；`DREAMS.md` 暂存 + `vole memory review` 提升；运行时端在 `compactMessages` 前触发 `append_daily_memory` 的 silent turn；六个新 prompt section（Reasoning、Reply Tags、Documentation、Self-Update、Execution Bias、Current Date & Time）。
...
本次新增架构：`better-sqlite3` 依赖（通过 pnpm `onlyBuiltDependencies` 编译；在 CLI tsup 打包中标为 external）；`SqliteSessionStore` 与 `SqliteTaskFlowStore` 实现现有 `SessionStore` / `TaskFlowStore` 接口，启用 WAL 日志模式与适当索引；`drainPendingForParent` 现在作为单一 SQLite 事务运行，无需重写整个文件。

推迟到 Phase 14b：带 FTS5 与可选 `sqlite-vec` 的 SQLite 记忆索引（被 Phase 13 混合 `memory_search` 阻塞）；`vole migrate jsonl-to-sqlite` 命令含备份与行数校验；启动迁移提示。
