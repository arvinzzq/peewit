...
| Phase 14 | 已完成 | SQLite 存储统一升级 | 全部 8 步已交付（Step 5、6、7 落在 14b） | SQLite store、SqliteMemoryIndex（FTS5）、vole migrate jsonl-to-sqlite、启动迁移提示 |
| Phase 15 | 部分 | Channels 与多 Agent 身份 | per-agent 身份 + `vole agents` + channel↔submitter 桥已交付；Telegram + Email 后端仍需外部基础设施 | agents/<id>/ 身份、vole agents CLI、channel 桥、Telegram（推迟）、Email（推迟） |
| Phase 16 | 已完成 | 沙箱与插件运行时 | 全部 7 步已交付（Step 3、4、6 落在 16b） | SandboxBackend、WorkspaceSandbox、WorkerThreadSandbox、DockerSandbox、vole doctor + --fix |
...
状态：已完成。计划文档：[phase-14-sqlite-storage-unification.zh-CN.md](../plans/phase-14-sqlite-storage-unification.zh-CN.md)。

目标：把所有持久化 store 从 JSONL 迁移到 SQLite —— sessions、TaskFlow 记录、记忆索引 —— 带 FTS5 关键词搜索、索引化查询，以及原子多记录更新。

本次（Phase 14 + 14b 全量）新增架构：`better-sqlite3` 依赖；`SqliteSessionStore` 与 `SqliteTaskFlowStore`（WAL 日志）；`drainPendingForParent` 单事务实现；`SqliteMemoryIndex`（FTS5 段落索引，按内容 hash 幂等 reindex）；`migrateJsonlSessionsToSqlite` / `migrateJsonlTaskFlowToSqlite` 辅助函数；抽出 `SQLITE_SESSIONS_SCHEMA_SQL` / `SQLITE_TASKFLOW_SCHEMA_SQL` / `SQLITE_MEMORY_INDEX_SCHEMA_SQL` DDL 常量；`vole migrate jsonl-to-sqlite` CLI（dry-run + `--apply`）；交互启动时的迁移提示。
...
本次（Phase 15 + 15b）新增架构：新 `docs/architecture/channels.md`；`multi-agent-runtime.md` 双语 Phase 15 提示；`@vole/channels` package，含 `Channel`、`ChannelRegistry`、`FakeChannel`、`sessionKeyForInbound`；`@vole/config` 中的 per-agent 身份加载器（`isValidAgentId`、`listAgentDirectories`、`resolveActiveAgentId`、`loadAgentIdentity`、`createAgentDirectory`、`setActiveAgentId`、`archiveAgentDirectory`）+ `agents.default` + `VOLE_AGENT` 环境变量；`vole agents list / create / switch / remove --confirm` CLI；channel↔submitter 桥（`createGatewayInboundHandler`、`bridgeRegistryToSubmitter`）让入站 channel 消息经 adapter 提供的 submitter 转发，channels 与 gateway 保持解耦。

仍推迟（外部基础设施）：Telegram 后端（`@vole/channels-telegram`）需要长轮询 bot 客户端 + mock-server 测试 harness；Email 后端需要 IMAP/SMTP 客户端 + 内嵌邮件测试 harness。它们落地后，channel adapter 把它们注册到 `ChannelRegistry` 并通过 `bridgeRegistryToSubmitter` 接入即可，无需再做架构变更。

非目标：不做 Slack / Discord / WhatsApp / webhook channel（Phase 17+）；不做跨 Agent 直接调用；不做托管多租户部署；不做 Agent 进程隔离。

## 20. Phase 16：沙箱与插件运行时

状态：已完成。计划文档：[phase-16-sandbox-and-plugin-runtime.zh-CN.md](../plans/phase-16-sandbox-and-plugin-runtime.zh-CN.md)。

目标：用真正的沙箱后端（workspace、Docker、worker thread）取代单一布尔；worker-thread 隔离的插件运行时让不可信 skill 不会弄崩主进程；`vole doctor` 自维护。

本次（Phase 16 + 16b）新增架构：`sandboxing.md` 与 `plugin-system.md` 的双语 Phase 16 提示；`@vole/permissions` 中的 `SandboxBackend` 接口，含 `SandboxCommand` / `SandboxOptions` / `SandboxResult` 值类型，以及三个实现 —— `WorkspaceSandbox`（workspace-escape 拒绝、cwd 包含、timeout、非零 exit 传播）、`WorkerThreadSandbox`（基于 node:worker_threads，带 timeout + 内存上限）、`DockerSandbox`（每次执行 `docker run --rm`，workspace 只读挂载，默认 deny 网络，gated on daemon 可用性）；顶层 CLI 命令 `vole doctor`，含只读检查与 `--fix` 幂等修复（删除 stale `.lock`、取消僵死子代理、取消孤儿 TaskFlow 子任务）。
