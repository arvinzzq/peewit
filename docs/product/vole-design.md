...
Simplified Chinese version: [vole-design.zh-CN.md](./vole-design.zh-CN.md)

## 1. Project Intent

Vole is intended to become a real, usable personal general-purpose agent product, while also serving as a learning project for understanding the architecture and implementation principles behind systems like OpenClaw.
...
Vole should eventually support multiple user entry points:
...
Vole should use a lightweight monorepo from the beginning.
...
Vole should support multiple execution modes because product usage and learning usage need different levels of visibility and interruption.
...
```text
VOLE_API_KEY       openai-compatible
...
- Project config: `vole.config.json` in the current workspace
...
Secrets should not be written into project configuration files. API keys should be provided through environment variables such as `VOLE_API_KEY`.

The `vole config` command should show the effective configuration while hiding secret values. A future interactive `/config` slash command may show the same redacted view inside chat.
...
Vole should treat context assembly as a first-class architecture module, following OpenClaw's separation between runtime orchestration and context construction.
...
- Interactive chat mode, such as `vole chat`
- Single task mode, such as `vole run "<goal>"`
...
The roadmap should evolve Vole from a small but usable CLI MVP into a full personal agent platform.
