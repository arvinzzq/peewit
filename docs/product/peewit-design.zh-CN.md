...
English version: [peewit-design.md](./peewit-design.md)

## 1. 项目意图

Peewit 旨在成为一个真实可用的个人通用 Agent 产品，同时也作为一个学习项目，用来理解 OpenClaw 这类系统背后的架构和实现原理。
...
Peewit 应该支持多种执行模式，因为产品使用和学习使用需要不同层级的可见性和打断频率。
...
- 项目配置：当前 workspace 中的 `peewit.config.json`
...
密钥不应写入项目配置文件。API keys 应通过环境变量提供，例如 `PEEWIT_API_KEY`。

`peewit config` 命令应展示当前生效配置，同时隐藏密钥值。未来交互式 `/config` slash command 可以在 chat 内展示同一个脱敏视图。
...
Peewit 应把 context assembly 视为一等架构模块，遵循 OpenClaw 将 runtime orchestration 与 context construction 分离的方式。
...
- 交互式 chat 模式，例如 `peewit chat`
- 单次任务模式，例如 `peewit run "<goal>"`
...
Roadmap 应将 Peewit 从小而可用的 CLI MVP 演进成完整个人 Agent 平台。
