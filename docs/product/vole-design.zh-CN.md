...
English version: [vole-design.md](./vole-design.md)

## 1. 项目意图

Vole 旨在成为一个真实可用的个人通用 Agent 产品，同时也作为一个学习项目，用来理解 OpenClaw 这类系统背后的架构和实现原理。
...
兼容性决策：[docs/decisions/0002-openclaw-aligned-not-identical.zh-CN.md](../decisions/0002-openclaw-aligned-not-identical.zh-CN.md)
...
- `packages/config`：配置加载、验证、优先级和脱敏
...
Vole 应该支持多种执行模式，因为产品使用和学习使用需要不同层级的可见性和打断频率。
...
详细架构说明：[docs/architecture/configuration-system.zh-CN.md](../architecture/configuration-system.zh-CN.md)

MVP 应使用配置文件保存非敏感设置，并使用环境变量保存密钥。

配置层：

- 项目配置：当前 workspace 中的 `vole.config.json`
- 用户配置：`~/.vole/config.json`
- 用于密钥和覆盖项的环境变量
...
密钥不应写入项目配置文件。API keys 应通过环境变量提供，例如 `VOLE_API_KEY`。

`vole config` 命令应展示当前生效配置，同时隐藏密钥值。未来交互式 `/config` slash command 可以在 chat 内展示同一个脱敏视图。
...
Vole 应把 context assembly 视为一等架构模块，遵循 OpenClaw 将 runtime orchestration 与 context construction 分离的方式。
...
- 交互式 chat 模式，例如 `vole chat`
- 单次任务模式，例如 `vole run "<goal>"`
...
Roadmap 应将 Vole 从小而可用的 CLI MVP 演进成完整个人 Agent 平台。
