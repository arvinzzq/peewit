...
本文档记录 Peewit 当前对 OpenClaw 架构和实现的理解。
...
- 基于文档和文件名做出的实现推断
- Peewit 从这些发现中得到的设计决策

这种区分很重要，因为 Peewit 的目标是实现一个 OpenClaw-like 系统，而不只是模仿表面功能。
...
这支持 Peewit 当前计划：避免把所有 skill bodies 注入每次 model call。

### Memory

OpenClaw 的 memory system 比 Peewit 的 MVP 计划更高级。

Peewit 不应该立即实现 OpenClaw 的完整 memory stack。分阶段计划应是：
...
Peewit 应把 prompt assembly 做成一等模块。
...
Peewit 应分阶段支持 OpenClaw-like workspace files：
...
Peewit 应保持 long-term memory 可见、可编辑、可审查，而不是隐藏在 opaque state 中。

### Skills

Peewit 应遵循 OpenClaw 文档化的方法，让 skill bodies 按需加载。

System prompt 应包含 compact skill index，而不是每个完整 skill instruction。

### Hooks

OpenClaw 有很多 hook points。Peewit 应将 hooks 延后到 tool、permission、trace 和 context assembly 稳定之后。
...
Peewit 影响：我们的 `ContextSkillSummary.when` 字段不是标准字段。正确方式是单一 `description` 字段同时回答"这是什么"和"何时使用"。`when` 字段应被移除，其内容合并到 `description` 中。
...
**Peewit 影响**：预执行 `Plan` 结构（基础设施驱动的分步执行）已被移除。正确方式是模型调用的 `update_todos` tool 等价物 — 已由第三轮研究（第 8 节）确认。TaskFlow 等价的持久化（SQLite、跨 session）属于 Phase 8+。
...
**与已移除 Peewit Plan 的关键区别：** 模型在执行*过程中*调用 `update_plan` 追踪已完成的工作 — 而不是在执行*之前*生成供 runtime 编排的计划。该 tool 不包含任何基础设施侧的执行管理。
...
Peewit 影响：按此模型调用模式实现 `update_todos` tool。不需要基础设施编排。
...
Peewit 影响：这是 Phase 4 值得加入的高价值机制。没有它，模型可能无限叙述计划而不采取行动。该检查应在 `AgentRuntime` 中每次模型响应后、tool dispatch 前执行。
...
Peewit 影响：Subagents 属于 Phase 7+ 工作，需要 gateway 和 multi-session 基础设施。不在早期 phases 实现。
...
Peewit 影响：暂缓。Anthropic 模型在内部处理此事。Phase 9+ 之前不需要配置接口。
...
## 11. Peewit Backlog 更新

本研究建议新增或细化这些 Peewit 文档：
