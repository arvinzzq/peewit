...
| Part | Commit | 内容 |
| --- | --- | --- |
| A | `docs: add Phase 9 design — plugin and skill ecosystem` | 本计划、plugin-system.md、skill-permissions.md、roadmap 更新 |
| B | `feat(skills): add extended metadata, SkillManager install/enable/disable/trust` | packages/skills 更新，含 SkillManager、扩展 SkillDefinition、测试、文档 |
| C | `feat(cli): add skills install, enable, disable, trust, review subcommands` | apps/cli skills 子命令、测试、文档 |
...
- `arvinclaw skills` — 列出所有 skills，含 version、trust 状态、permissions
- `arvinclaw skills install <path>` — 从本地 .md 文件安装
- `arvinclaw skills enable <name>` — 启用已禁用的 skill
- `arvinclaw skills disable <name>` — 禁用已启用的 skill
- `arvinclaw skills trust <name>` — 标记为 trusted
- `arvinclaw skills review <name>` — 展示完整 metadata

所有子命令使用来自 `@arvinclaw/skills` 的 `SkillManager`。
...
- `arvinclaw skills trust <name>` 将 skill 标记为 trusted。
