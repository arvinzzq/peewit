...
- `peewit skills` — 列出所有 skills，含 version、trust 状态、permissions
- `peewit skills install <path>` — 从本地 .md 文件安装
- `peewit skills enable <name>` — 启用已禁用的 skill
- `peewit skills disable <name>` — 禁用已启用的 skill
- `peewit skills trust <name>` — 标记为 trusted
- `peewit skills review <name>` — 展示完整 metadata

所有子命令使用来自 `@peewit/skills` 的 `SkillManager`。
...
- `peewit skills trust <name>` 将 skill 标记为 trusted。
