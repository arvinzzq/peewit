...
这个阶段让 Vole 拥有 short-term 和 durable memory。
...
如果未指定 session，CLI 会创建一个通用的 `session_<id>` session。Session ID 是 agent-level identifier，不应该编码 CLI 或 Web UI 这样的 entry adapter。默认 storage directory 是 `~/.vole/sessions`。

Configured CLI chat 也会在文件存在时，从 configured workspace root 加载 `AGENTS.md` 和 read-only `SOUL.md`。Workspace root 可以通过 `VOLE_WORKSPACE_ROOT` 设置。

Long-term memory files 默认保持 disabled。`VOLE_LONG_TERM_MEMORY=read-only` 会在文件存在时，从 configured workspace root 加载 `USER.md`、`MEMORY.md`、`memory/<today>.md` 和 `memory/<yesterday>.md`，但 memory writes 仍保持 disabled。
