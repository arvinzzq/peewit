...
此 resolver 与 `@vole/core` 中现有的 `ApprovalResolver` 接口集成。无需更改 `AgentRuntime`。
...
- `vole run "<goal>"` 执行任务并打印紧凑 trace。
- `vole run "<goal>" --mode auto` 使用自动审批策略。
- `vole tasks` 列出带状态的最近任务运行。
- 后台任务将运行记录持久化到 `task-runs.jsonl`。
- 危险动作在后台模式下遵守 permission policy。
- 失败的后台任务在 `vole tasks` 输出中可见。
