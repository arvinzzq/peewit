...
Phase 0–10 已将 Peewit 建立为与 OpenClaw 核心架构对齐的可用个人 Agent 平台。

本文档追踪了迭代 1–7 中 Peewit 与 OpenClaw 生产行为之间的剩余差距。所有 18 个差距现已全部关闭。
...
| 13 | Cron Daemon（`peewit daemon`） | 🟡 中 | 5 | ✅ 已完成 | `6f47106` |
...
长对话会溢出模型的上下文窗口。Peewit 目前每轮都传递所有消息，没有任何管理。
...
Peewit 目前将所有 SKILL.md 全文注入每个 Prompt。Skill 数量多时会大量浪费 Token。
...
OpenClaw 支持三种 Prompt 渲染模式。Peewit 始终渲染所有段落。
...
- `full`：所有段落（当前行为，保持默认）
- 通过 `PEEWIT_PROMPT_MODE` 环境变量和 `runtime.promptMode` 配置字段暴露
...
OpenClaw 加载 `TOOLS.md`、`IDENTITY.md`、`HEARTBEAT.md`、`BOOTSTRAP.md` 作为启动文件。Peewit 目前只加载 `AGENTS.md` 和 `SOUL.md`。
...
- 通过 `PEEWIT_EXECUTION_CONTRACT` 环境变量和配置暴露
...
OpenClaw 为扩展提供了多个 Hook 点。Peewit 目前没有 Hooks。
...
- 通过 `PEEWIT_TOOL_PROFILE` 环境变量可配置
...
`peewit run` 执行单个任务，没有循环调度器。

Peewit 设计：
- `peewit daemon` 命令 — 启动长期运行进程
...
- CLI 命令：`peewit taskflow list`、`peewit taskflow show <id>`、`peewit taskflow cancel <id>`
...
Peewit 设计：
- Dreaming 是一种特殊的后台任务：`peewit run --dream`
