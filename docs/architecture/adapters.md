...
ArvinClaw exposes one `AgentRuntime` and multiple surfaces: terminal (CLI), browser (Web UI), and eventually desktop, messaging, and background tasks. The adapter layer keeps those surfaces interchangeable without duplicating agent logic.
...
The CLI adapter uses readline (or Ink for live streaming) to collect input and write output. It uses `JsonlSessionStore` at `~/.arvinclaw/sessions` for durable session history. It supports streaming output and interactive approval prompts.
