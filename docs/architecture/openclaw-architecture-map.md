...
Vole's target is to implement an OpenClaw-like personal general-purpose agent system from zero to one.

This document maps OpenClaw concepts to Vole phases so the project can intentionally grow toward that target without overloading the MVP.

The core rule:

OpenClaw is the primary architecture reference. Vole implements its core ideas in staged, testable increments.
...
| OpenClaw Concept | Vole Plan | Phase |
| --- | --- | --- |
| Agent workspace | Local Vole workspace with prompt, memory, skill, and config files | Phase 0-1 |
...
Vole should implement a workspace model with:
...
Vole should support OpenClaw-like prompt files with explicit scope and safety.
...
Vole should mirror OpenClaw's plain-file memory direction, but phase it carefully.
...
OpenClaw-like agents are powerful because they can act. Vole should make action safe from the beginning.
...
Vole should implement startup loading in stages.
...
Vole should reach this through adapters:
...
Vole should implement this later through:
...
Vole should treat these as design requirements, not afterthoughts.
...
Each OpenClaw concept added to Vole should arrive with tests.
...
- Each major OpenClaw concept has an Vole phase.
