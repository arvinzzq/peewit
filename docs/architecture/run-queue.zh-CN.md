...
Run queue 控制 ArvinClaw 如何接受、排序、执行和持久化 agent runs。

OpenClaw 调研显示，runs 会按 session 序列化，并通过 queues 和 session write locks 协调。ArvinClaw 应分阶段采用这个架构。
...
Run queue 给 ArvinClaw 提供可预测执行和持久化行为。
...
ArvinClaw 最终应按 session 序列化 runs。
...
ArvinClaw 应延后 steering messages。
...
在实现 scheduled tasks 或 heartbeat behavior 前，ArvinClaw 需要：
