...
Background automation lets ArvinClaw run agent tasks without a foreground user connection.
...
The `BackgroundApprovalResolver` implements the `ApprovalResolver` interface from `@arvinclaw/core`.
...
Each invocation of `arvinclaw run` follows this lifecycle:
