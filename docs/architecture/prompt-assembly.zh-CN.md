...
OpenClaw 调研说明，prompt construction 是核心架构关注点，不是 CLI 细节。Peewit 应遵循这个方向，让 prompt assembly 显式、可测试、在 trace 中可见，并与 entry adapters 解耦。
...
Prompt assembly 给 Peewit 提供了所有 model-facing context 的受控边界。
...
Peewit 只有在 section 行为可以被文档化和测试时才应增加它。
