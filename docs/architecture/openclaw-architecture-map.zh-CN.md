...
Vole 的目标是从 0 到 1 实现一个 OpenClaw-like 的个人通用 Agent 系统。

本文档把 OpenClaw 概念映射到 Vole phases，让项目可以有意识地朝这个目标成长，同时不让 MVP 过载。

核心规则：

OpenClaw 是 primary architecture reference。Vole 以分阶段、可测试的增量实现它的核心思想。
...
| --- | --- | --- |
| Agent workspace | 包含 prompt、memory、skill 和 config files 的本地 Vole workspace | Phase 0-1 |
...
Vole 应支持 OpenClaw-like prompt files，并带有明确 scope 和 safety。
...
Vole 应镜像 OpenClaw 的 plain-file memory 方向，但要谨慎分阶段。
...
OpenClaw-like agents 强大是因为它们可以行动。Vole 应从一开始就让行动安全。
...
每个加入 Vole 的 OpenClaw concept 都应带着测试一起到来。
...
- 每个主要 OpenClaw concept 都有 Vole phase。
