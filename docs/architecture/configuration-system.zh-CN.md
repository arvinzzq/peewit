...
Configuration system 定义 ArvinClaw 如何加载非敏感设置、secret references、默认值和 runtime overrides。
...
1. 内置默认值
2. 用户配置：`~/.arvinclaw/config.json`
3. 项目配置：`<workspace>/arvinclaw.config.json`
4. 环境变量
5. CLI flags
...
- `arvinclaw.config.json` 配置 runtime behavior。
