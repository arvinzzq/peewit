...
在 Peewit 中，这个循环要足够简单，便于学习；也要足够结构化，能够演进成真实的通用 Agent 平台。
...
Peewit Phase 4 实现等价的 `update_todos` tool，遵循同样的模式。
...
Peewit Phase 4 在 `AgentRuntime` 中添加等价的停滞检测。
...
这属于 Peewit Phase 7+，需要 multi-session 基础设施和 gateway。
