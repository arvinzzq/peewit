...
默认值：`maxMessages: 30`。可通过 `CompactionOptions` 调整。
...
成功 compaction 后会发出 `compaction_triggered` 事件，记录：

- 原始消息数量
- Compaction 后的消息数量
- 摘要字符长度
- 是否遵守了 keepRecent
- `summary: string` — 从压缩后消息中提取的摘要文本（Phase 2 成功时存在）

## 6. 故障保护

如果摘要模型调用因任何原因失败（网络错误、模型拒绝、超时），compaction 函数会捕获错误，记录警告 trace 事件，并返回 **Phase 1 精简后的消息**——而非原始消息。

Phase 1（机械精简）已经用摘要替换了大型工具结果内容。这些精简后的消息比原始消息小，可以安全用作上下文。Phase 2 调用失败后，工具输出内容永远不会恢复到上下文中。

Agent turn 会继续使用精简后的消息列表。这比使 turn 失败或静默截断消息更安全。
...
| 故障保护直通 | 错误捕获 + 返回 Phase 1 精简后的消息 |

OpenClaw 的 compaction 还为每个 session 追踪 `compactionCount` 以提高可观察性。Vole 通过 `compaction_triggered` 事件实现相同功能。

## 7a. Compaction 结果持久化

Compaction 成功后，CLI 适配器监听 `compaction_triggered` 事件，若 `event.summary` 非空，则调用 `session.appendCompactBoundary({ summary, ... })`。

这向 session JSONL 文件写入一行 `compact_boundary` 记录：

```jsonl
{"type":"compact_boundary","summary":"Conversation summary:\n…","messagesBefore":35,"messagesAfter":14,"createdAt":"…"}
```

下次 session 加载时，`JsonlSessionStore` 中的 `#replay()` 遇到该记录，执行：
1. 清空已积累的 `messages` 数组。
2. 在位置 0 插入 `{ role: "system", content: summary }` 合成消息。

这意味着 compaction 在每次 session boundary 处只执行一次——不会在每次加载时重新计算。Session 文件是 append-only 的；boundary 之前的旧消息仍保留在磁盘上供手动查看，但 `listMessages()` 不会返回它们。

CLI 适配器也移除了 `listMessages()` 上的 `limit: 12` 限制。`compact_boundary` 重放机制自动截断旧消息，因此不需要人为限制。
...
- 成功时发出 `compaction_triggered` 事件，含 `summary` 字段。
- 如果摘要模型调用失败，返回 Phase 1 精简后的消息（而非原始消息）。
