...
Vole's sandbox is implemented through tool-level guards in `packages/tools`. There is no OS-level process isolation. The sandbox is a set of input validation and path restriction rules enforced before any filesystem or shell operation.
...
- Command runs as the same user as the Vole process — no privilege escalation.
