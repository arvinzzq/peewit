...
1. `vole skills install <path>` — 复制文件，记录 `trusted: false`。
2. `vole skills review <name>` — 展示完整 metadata，包括声明的 permissions。
3. 用户阅读 permissions 和 skill 正文。
4. `vole skills trust <name>` — 在 manifest 中记录 `trusted: true`。
...
Trust 标志表达"我已阅读此 skill 并认为其安全"。它不强制隔离或对文本进行沙箱处理。实际的工具 permission 决策仍通过 `@vole/permissions` package 进行，与 skill trust 无关。

## 5. CLI 审查命令

```
vole skills             列出所有 skills — 显示 version、trust 标志、permissions
