...
Skill System 给 ArvinClaw 提供一种通过可复用指令扩展 Agent 行为的方式。
...
格式应是简单 Markdown。MVP 可以使用一个小的 frontmatter block 加正文内容。
...
系统应避免把每个完整 skill 盲目塞进每次模型调用。过大或无关的 skill 文本会浪费上下文，也可能让行为混乱。
