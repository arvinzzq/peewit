...
Vole defines five hook types aligned with the agent lifecycle:
...
OpenClaw's hooks are used for memory consolidation triggers, channel-specific logging, and policy audit trails. Vole adopts the same pattern with explicit TypeScript interfaces.
