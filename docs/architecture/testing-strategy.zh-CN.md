...
测试是 Vole 架构的一部分，不是实现之后的清理工作。

Vole 既是可用的 Agent 产品，也是学习项目。测试应该保护行为、解释模块边界，并让未来重构更安全。
...
| CLI adapter tests | 验证用户可见工作流 | `vole chat`、permission prompt flow、slash commands |
