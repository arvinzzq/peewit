...
| 命令 | 用途 | 阶段 |
| --- | --- | --- |
| `arvinclaw chat` | 开始一个交互式 chat session | Phase 1 |
...
1. 加载配置。
2. 创建或恢复一个轻量 session。
3. 为每个 user turn 创建 run ID。
4. 将用户消息发送给 Agent Core。
5. 流式输出或打印 assistant output。
6. 当 trace events 到达时渲染它们。
7. 当 core 报告 approval request 时询问 permission。
8. 当 storage 存在后持久化 session 和 trace data。
