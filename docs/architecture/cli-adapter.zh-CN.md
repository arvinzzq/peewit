...
CLI adapter 是 Peewit 第一个面向用户的产品表面。
...
Peewit 从 CLI 开始，因为它是最小但有用的产品表面，也是最容易研究 Agent 内部机制的地方。
...
| 命令 | 用途 | 阶段 |
| --- | --- | --- |
| `peewit chat` | 使用 configured provider settings 开始一个交互式 chat session | Phase 1 |
| `peewit chat --session <id>` | 开始或继续一个 JSONL-backed named chat session | Phase 5 |
| `peewit chat --resume` | 继续最近更新的 JSONL-backed chat session | Phase 5 |
| `peewit chat --fake-interactive` | 使用 fake provider 开始一个本地学习用交互 session | Phase 1 |
| `peewit chat --fake "<message>"` | 运行一次 fake-provider smoke path | Phase 1 |
| `peewit sessions` | 列出 stored JSONL chat sessions | Phase 5 |
| `peewit --version` | 显示版本 | Phase 0-1 |
| `peewit --help` | 显示可用命令 | Phase 0-1 |

早期后续命令：

| 命令 | 用途 | 阶段 |
| --- | --- | --- |
| `peewit run "<goal>"` | 运行一次性 goal 后退出 | Phase 2-4 |
| `peewit trace <session>` | 检查已存储 trace events | Phase 5 |
| `peewit skills` | 列出已加载 skills | Phase 3 |
| `peewit config` | 检查生效配置 | Phase 1-2 |

MVP 应避免过大的命令表面。命令应该在底层模块存在并有测试保障后再出现。

## 5. 交互式 Chat

`peewit chat` 是 MVP 的主要工作流。

预期行为：

1. 加载配置。
2. 对 configured provider path 要求 `PEEWIT_API_KEY` 或 `OPENROUTER_API_KEY`。
3. 创建或恢复一个轻量 session。
...
Configured chat 默认把 messages 存入 `~/.peewit/sessions` 下的 JSONL session files。Named sessions 使用 `--session <id>`，并且必须使用 safe session IDs。`--resume` 会选择最近更新的 stored session 并继续它。默认 session IDs 使用通用的 `session_<id>` 形态，因为 sessions 属于 Agent，而不是某个特定 adapter。
...
后续 `peewit trace <session>` 可以在 session 结束后检查已存储 traces。
...
CLI 可以通过 `peewit config` 展示生效的非 secret 配置。
...
Ink 是一个基于 React 的终端 UI 框架，允许组件在原地渲染和重新渲染。它适合 Peewit 的原因：
...
- `peewit chat` 可以启动一个可用的 interactive session。
