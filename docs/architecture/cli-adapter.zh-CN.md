...
| `arvinclaw chat --resume` | 继续最近更新的 JSONL-backed chat session | Phase 5 |
...
1. 加载配置。
2. 对 configured provider path 要求 `ARVINCLAW_API_KEY` 或 `OPENROUTER_API_KEY`。
3. 创建或恢复一个轻量 session。
...
Configured chat 默认把 messages 存入 `~/.arvinclaw/sessions` 下的 JSONL session files。Named sessions 使用 `--session <id>`，并且必须使用 safe session IDs。`--resume` 会选择最近更新的 stored session 并继续它。默认 session IDs 使用通用的 `session_<id>` 形态，因为 sessions 属于 Agent，而不是某个特定 adapter。
