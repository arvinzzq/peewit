...
Configured chat 默认把 messages 存入 `~/.arvinclaw/sessions` 下的 JSONL session files。Named sessions 使用 `--session <id>`，并且必须使用 safe session IDs。默认 session IDs 使用通用的 `session_<id>` 形态，因为 sessions 属于 Agent，而不是某个特定 adapter。
