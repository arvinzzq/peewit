...
- `read-only`：面向未来的安全模式；当 context loader 支持这些文件后，只允许只读加载。
...
| `USER.md` | User profile、preferences 和 durable user context | Policy gated, loading deferred |
...
MVP 从 `AGENTS.md` 风格 operational instructions、read-only `SOUL.md` 和 session storage 开始。未来 loader 要包含 `USER.md` 和 `MEMORY.md` 前，long-term memory policy 必须先切到 `read-only`。
