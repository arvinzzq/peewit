...
| Long-term memory | Designed, deferred | 跨 sessions 的 curated durable knowledge |
...
| `USER.md` | User profile、preferences 和 durable user context | Designed, deferred until long-term memory policy |
| `MEMORY.md` | Curated long-term memory | Deferred |
| `memory/YYYY-MM-DD.md` | Daily notes 和 recent observations | Deferred or Phase 5 |
| `TOOLS.md` | Environment and tool notes | Deferred |

MVP 应从 `AGENTS.md` 风格 operational instructions 和 session storage 开始。等 prompt assembly 和 security rules 清楚后，可以支持 `SOUL.md` 作为 read-only prompt file。
...
ArvinClaw 应在 session storage 和 trace 稳定后再做 daily memory files。
