...
Memory system 定义 Vole 如何在 turns、sessions，以及最终长期个人使用中保存有用上下文。

Vole 应该学习 OpenClaw 的 file-based workspace model，但应该分阶段实现 memory，让 MVP 保持可理解、可测试和安全。
...
Vole 应借鉴这种模型的清晰性，同时保持初始实现更小。

## 3. Vole Memory Layers

Vole 应使用四个 memory-related layers。
...
Vole 应为未来的 `MEMORY.md` 做设计，但不在第一版 MVP 中实现完整 long-term memory writes。
...
Vole 应支持受 OpenClaw 启发的 workspace prompt files，但要谨慎分阶段。
...
因为 `USER.md` 可能包含敏感个人信息，Vole 不应在 MVP 中实现对它的自动写入。
