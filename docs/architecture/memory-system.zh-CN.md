...
Memory system 定义 Peewit 如何在 turns、sessions，以及最终长期个人使用中保存有用上下文。

Peewit 应该学习 OpenClaw 的 file-based workspace model，但应该分阶段实现 memory，让 MVP 保持可理解、可测试和安全。
...
Peewit 应借鉴这种模型的清晰性，同时保持初始实现更小。

## 3. Peewit Memory Layers

Peewit 应使用四个 memory-related layers。
...
Peewit 应为未来的 `MEMORY.md` 做设计，但不在第一版 MVP 中实现完整 long-term memory writes。
...
Peewit 应支持受 OpenClaw 启发的 workspace prompt files，但要谨慎分阶段。
...
因为 `USER.md` 可能包含敏感个人信息，Peewit 不应在 MVP 中实现对它的自动写入。
...
OpenClaw 对齐：OpenClaw 将记忆 Dreaming 实现为使用专用记忆整合 Skill 的定时 Agent 运行。Peewit 实现此功能时应遵循相同模式。
