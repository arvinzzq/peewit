...
Peewit 使用轻量 monorepo，是为了让项目可以从 CLI MVP 演进到多入口 Agent 平台，而不需要重写核心架构。

核心规则：

Agent Core 拥有 Agent 行为。用户界面只负责把用户输入输出适配到核心。

这很重要，因为 Peewit 预期先支持 CLI，然后支持 Web UI、桌面应用、消息平台和后台自动化。如果第一版 CLI 拥有太多业务逻辑，后续每个入口都需要复制或反向拆解这些行为。
...
文档是产品目标的一部分。Peewit 应该既能运行，也能用于学习。
...
Peewit 不应该一开始就引入复杂发布、release 自动化或包治理。
