...
单个 `AgentRuntime` 在每个进程一个 session 时运行良好。随着 Vole 向个人 agent 平台演进，会出现几个问题：
...
`GatewaySession` 记录包含：`id`、`adapterName`、`capabilities`（来自 `@vole/adapters`）、`registeredAt` 和 `lastActivityAt`。
