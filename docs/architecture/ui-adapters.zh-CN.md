...
ArvinClaw 有一个 Agent 运行时（`AgentRuntime`）和多个交互界面：终端、浏览器，以及未来的桌面端和消息端。适配器层让这些界面可以互换，而无需复制 Agent 逻辑。
...
- CLI：`JsonlSessionStore`，存储于 `~/.arvinclaw/sessions`。
- Web：初始使用 `InMemorySessionStore`；可升级为 `JsonlSessionStore` 或数据库。
- 测试：使用可控会话 ID 的 `InMemorySessionStore`。

`SessionStore` 和 `RuntimeTraceStore` 接口分别定义在 `@arvinclaw/sessions` 和 `@arvinclaw/core` 中。适配器在启动时选择具体实现。
