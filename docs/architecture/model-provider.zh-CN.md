...
Vole 应该可以先接入一个 OpenAI-compatible provider，后续再支持 Anthropic、Gemini、Ollama、本地模型或其他托管模型，而不需要重写 Agent Loop。
...
- 把 Vole 的模型输入转换成 provider 请求格式
- 把 provider 输出转换成 Vole 模型输出
- 归一化工具调用
