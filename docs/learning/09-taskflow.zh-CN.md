...
`@vole/sessions` 有 `InMemorySessionStore`，是为了方便测试。Sessions 包的内部逻辑有足够的复杂度——replay、`compact_boundary` 处理、`updatedAt` 推导——内存版本让这些测试更干净，不需要涉及文件系统。注意：生产环境中始终使用 `JsonlSessionStore`，`InMemorySessionStore` 只出现在 `createFake()` 这个专门给测试使用的工厂方法里。

`@vole/taskflow` 没有对应的内存版本，是因为 `JsonlTaskFlowStore` 本身已经优雅地处理了文件缺失的情况——`#readAll()` 在文件不存在时返回 `[]`，第一次 `create()` 调用会透明地创建文件。测试成本因此几乎和内存版本一样低：`mkdtemp` 创建临时目录是唯一的准备工作。没有足够复杂的逻辑需要一个独立的内存实现。
