...
**通俗解释**：想象 agent 循环是一位厨师，需要食材。它不在乎食材来自超市、农场还是进口
店——只需要食材以标准格式到达：「这是对话消息，给我一个回复。」

`@vole/models` 是这个物流层。它用 agent 的内部格式接收订单，处理所有厂商特定的手续
（API 密钥、消息格式转换、流式协议、错误码），把结果以厨师认识的统一格式返回。把
Anthropic 换成 OpenAI——厨师完全感知不到变化。
...
**技术说明**：`@vole/models` 把所有厂商特定的模型 API 封装在统一的 `ModelProvider`
接口后面。它把内部的 `ModelMessage[]` 格式翻译成各厂商的线路格式，调用 API，再把响应
归一化回 `ModelOutput`。
