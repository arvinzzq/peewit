...
## 9. Phase 16 更新 —— SandboxBackend 也住在这里

Phase 16 让 permissions package 承担了第二个职责：**执行边界抽象**。Policy 仍然在工具运行*前*决定 allow/ask/deny，但对于执行外部代码的工具（`run_shell`、不可信 skill、未来的 code-eval 工具）现在还多了一个问题——*如何*运行。本 package 中的 `SandboxBackend` 接口给出答案：
...
`SandboxResult` 是带 tag 的联合 —— 常规拒绝（`reason: "rejected" | "timeout" | "unavailable"`）作为显式的非完成结果暴露出来，而非抛异常，所以调用方总会处理。

`WorkspaceSandbox` 是 Phase 16 交付的参考后端。它将 `cwd` 钉到 workspace root，拒绝匹配 workspace-escape 启发式的 shell 片段，遵守 `timeoutMs`。`DockerSandbox`（每次执行起容器）与 `WorkerThreadSandbox`（JS skill 隔离）推迟到 Phase 16b，但将实现同一接口 —— 调用方代码无需更改。

**为何与 policy 同居一个 package**：两者都在推理执行边界。Policy 回答"这次工具调用根本上是否允许？"；后端回答"如果运行，在什么隔离里？"。把它们放在一个 package 里，让你无法只观察其中一个而绕过另一个。

**心智模型**：`policy.evaluate` → "我们可以吗？"；`sandbox.execute` → "现在做，但有边界"。Phase 16 接线完成后，工具不再直接调 `child_process.spawn`；它们调 `sandbox.execute(...)`。
