...
**核心问题**：阅读源码时回答这些问题：
- 19 个 `RuntimeEventType` 值是什么，正常运行时以什么顺序出现？
...
**通俗解释**：把 `@vole/core` 想象成一个项目经理。你给它一个任务，它不自己干活——
而是协调各个专家：

1. 「Context 组，准备发给模型的材料」
2. 「模型，下一步该做什么？」
3. 「权限组，这个操作允许吗？」
4. 「工具组，执行这个操作」
5. 「Context 组，把结果加入材料」
...
每一步都对外广播正在发生的事情（事件），让任何观察者——CLI、Web 界面、追踪日志——
都能实时响应，而不需要被硬编码进循环本身。

**技术说明**：`@vole/core` 运行 agent turn 循环。它接收用户消息，协调 context 组装、
模型推理、权限评估、工具执行——在一个循环中——并在每一步以 19 种可观察事件类型的
事件流向外发射。
...
1. 如果配置了 compaction，对消息历史进行压缩。如果压缩运行且成功，发出 `compaction_triggered` 事件，其中包含 `summary: string` 字段，内容是从压缩后消息中提取的摘要文本。
...
如果没有停顿：yield `assistant_message_created`，然后 yield `turn_complete`（携带本轮所有新消息——user、tool_use、tool_result 和最终 assistant），最后 yield `run_completed`，return。
