...
`@vole/sessions` 有 `InMemorySessionStore`，是因为 sessions 的内部逻辑比较复杂：replay、`compact_boundary` 处理、`updatedAt` 推导。内存版本让测试可以在不涉及文件系统的情况下验证这些逻辑。每个测试创建一个全新的内存 store，测完直接丢弃，无需清理。

`@vole/taskflow` 没有对应的内存版本，是因为 `JsonlTaskFlowStore` 本身已经优雅地处理了文件缺失的情况——`#readAll()` 在文件不存在时返回 `[]`，第一次 `create()` 调用会透明地创建文件。测试成本因此几乎和内存版本一样低：`mkdtemp` 创建临时目录是唯一的准备工作。再维护一个内存版本只会增加复杂度，没有实际收益。
