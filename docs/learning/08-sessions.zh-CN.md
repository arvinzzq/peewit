...
状态：已完成 · Phase 11 Step 4 加入跨进程文件锁
日期：2026-05-07；跨进程锁段落 2026-05-11 加入
...
请在阅读 [07-context.zh-CN.md](./07-context.zh-CN.md) 之后阅读本文档——上下文组装将会话消息作为 `recentMessages` 使用，而这些消息正是由本模块提供的。
...
- 一个会话文件有 50 条消息，`compact_boundary` 在第 20 条后面，`listMessages()` 返回几条？是哪些？
- 为什么 `SessionMutex` 在 `@vole/core` 而不是这里？（Phase 11+ 用 `@vole/lanes` 中的 session lane 替换）
- Phase 11 的跨进程文件锁住在本 package，而不是在 `@vole/lanes`。为什么不对称？
- `SessionMessageRecord` 和 `ModelMessage` 有什么区别？
- `@vole/sessions` 用追加写入，`@vole/taskflow` 用读取-修改-写入，各自为什么这样选择？

**检查点**：当你能够描述会话文件在磁盘上的样子，追踪 `listMessages()` 通过 `#replay()` 的全过程（包括 `compact_boundary` 的处理），并解释为什么 `@vole/sessions` 被称为"哑存储"时，说明你理解了本模块。
...
Phase 11 Step 4 在每次 `#append` 周围包一层 `acquireSessionFileLock`，让两个针对同一 workspace 的 `vole` 进程不会交错写入同一 JSONL 文件。

锁是一个旁车文件 `{directory}/{sessionId}.lock`，其内容是一个小型 JSON 文档：`{ pid, startedAt }`。获取使用 Node 的 `wx` 标志 —— atomic create-if-not-exists。若文件已存在，acquirer 轮询（默认每 50 ms）直到持有者释放或锁被判定为陈旧。陈旧的判据：`startedAt` 比 `staleAfterMs`（默认 60 s）更早，或持有 pid 不再存活（`process.kill(pid, 0)` 抛 `ESRCH`）。陈旧锁会被删除并由下一个 waiter 重新获取。总获取超时默认为 60 s。

`acquireSessionFileLock(lockPath, options)` 也作为独立辅助函数导出。它完全可注入（`pid`、`now`、`isProcessAlive`），便于确定性测试。

文件锁与 `@vole/lanes` 中的进程内 session lane 组合：session lane 在一个 Node 进程内排序写入；文件锁在多进程间排序写入。两层互不感知，可独立启用。
...
| 进程内 session lane | `SessionMutex`（Phase 10）→ `@vole/lanes` 的 session lane（Phase 11+） | 由 lane 组合替代 |
| 进程感知文件写锁 | `acquireSessionFileLock`（Phase 11 Step 4） | 语义相同：pid + startedAt 旁车、陈旧回收 |
...
sessions 包不拥有 per-session 进程内串行化。Phase 10 用 `@vole/core` 中的 `SessionMutex` 做这件事；Phase 11 把它替换为 `@vole/lanes` 中的 session lane（并发 1 是 mutex 的严格泛化）。原因：进程内顺序是*调度*关注点，取决于知道"什么是一个 run"——这是 gateway 与 runtime 的知识，不是存储层的。

跨进程串行化不一样。它仅依赖于文件路径，任何触及 JSONL 的进程都必须尊重它，与 run 如何调度无关。这让它成为*存储*关注点。因此文件锁住在 writer 旁边的 `packages/sessions`，可以自动应用到每次 `#append`。

两层组合：

- `@vole/lanes` session lane：一个进程内同一时刻一个 writer。
- `@vole/sessions` 文件锁：同一文件上同一时刻一个进程。
...
**项目作用域 sessions：CLI 适配器中的 git 根目录检测**

`@vole/sessions` 将 sessions 存储到给定的任意目录——它不感知 git 仓库。作用域逻辑在 CLI 适配器中：

1. 启动时，CLI 从 `process.cwd()` 向上遍历文件系统，寻找 `.git` 目录。
2. 如果找到，sessions 存储在 `<git-root>/.vole/sessions/`，使 sessions 与所属项目并置（如需要
   也可纳入版本控制）。
3. 如果未找到 git 根目录（例如在任何仓库之外运行），CLI 回退到全局 `~/.vole/sessions/` 路径。

这是纯粹的适配器层关注点：`@vole/sessions` 的 `JsonlSessionStore` 接收绝对目录路径并无条件写入。
`@vole/config` 包和 sessions 包都不感知 git；只有 CLI 适配器执行检测。
...
- 会话 ID 安全校验
- 缺失会话文件返回空（不报错）
- 文件锁：获取、幂等释放、活进程持有时超时、陈旧 pid 回收、陈旧时长回收、两个 acquirer 间的进程内串行化
- Store 层锁集成：`.lock` 旁车在每次 append 周围创建并清理；并发 append 正确串行化；`fileLock: { enabled: false }` 干净禁用该层
...
**Sessions 是"哑存储"——存储之上没有业务逻辑。** `@vole/sessions` 没有业务逻辑、没有并发保护、没有缓存，也不了解 agent 循环。所有关于写什么、何时写的决策都属于适配器（CLI）。这个边界是有意为之的：让 sessions 保持简单意味着它可以独立测试、替换或扩展，无需改动核心逻辑。

**追加写入（sessions）vs 读取-修改-写入（taskflow）——两种不同的数据语义。** Sessions 使用追加写入，因为对话历史是不可变的事件流——过去说过的话就是说过了，不会回头修改。Taskflow 使用读取-修改-写入，因为任务状态是可变实体——任务从 `queued` 转换到 `running` 再到 `succeeded`，只有当前状态重要。正确的写入策略直接由数据的本质决定。

**CLI 是桥梁——sessions 和 core 完全解耦。** `@vole/core` 发出 `turn_complete` 和 `compaction_triggered` 事件，但从不调用任何 session 方法。`@vole/sessions` 存储消息，但从不从 core 导入。只有 CLI 适配器同时了解两边：它监听 core 事件，决定持久化什么。这种解耦意味着 sessions 可以被替换（如 SQLite 后端）而不影响 core，core 也可以在没有文件系统的情况下测试。
...
2. `#replay()` 做了什么？如果会话文件有 50 条消息，`compact_boundary` 在第 20 条后面，`listMessages()` 返回几条？
   > 逐行读取整个会话文件，重建会话元数据、消息和追踪事件。遇到 `compact_boundary` 时，重置 messages 数组，插入摘要作为唯一的 `role: "system"` 消息，然后继续追加后续消息。结果：`listMessages()` 返回 31 条（1 条摘要 + 第 21～50 条）。第 1～20 条仍在文件里，但永远不会被返回。时间和 I/O 复杂度：每次读取操作 O(N)，无论查询哪些记录。
...
7. CLI 在 git 仓库内时将 sessions 存储在 `<git-root>/.vole/sessions/`。这是哪一层实现的——
   `@vole/sessions`、`@vole/config`，还是 CLI 适配器？为什么在那里而不是在 sessions 包中？
   > CLI 适配器。`@vole/sessions` 是一个哑存储，写入到它接收的任意目录。git 根目录检测是适配器层的
   > 部署决策，而非存储关注点。把它放在 sessions 包中会将存储逻辑与环境拓扑知识混合，并迫使每个
   > 消费者（Web、测试）携带它们不需要的 git 检测代码。
