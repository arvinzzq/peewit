...
Vole 的目标是构建一个 OpenClaw-like 的个人通用 Agent。

OpenClaw 是 primary architecture reference，但 Vole 也是学习项目，并且是供我们自己使用的产品。这意味着盲目复制每个实现细节不是正确目标。
...
如果 OpenClaw 某个成熟实现对 Vole 当前阶段来说过于复杂，Vole 应先实现更简单版本。
...
Vole 应保留 `SOUL.md`、`USER.md`、`MEMORY.md` 和 daily memory files 这些概念。

Vole 不需要立即匹配 OpenClaw 的完整 memory engine、hybrid search 或 dreaming behavior。

### Gateway

Vole 应保留长期 gateway 和 multi-entry 方向。

Vole 不需要在 MVP 实现 production-grade gateway。

### Skills

Vole 应保留本地 `SKILL.md` skills，并在后续演进到 plugins。

Vole 可以从更小的 skill loader 和更简单的 prompt integration 开始。

### Context Engine

Vole 应把 context assembly 作为一等概念。

Vole 可以先实现一个简单 deterministic assembler，再引入 plugin-provided context engines。

## 7. 后果

正向影响：

- Vole 保持与 OpenClaw 的形态对齐。
- MVP 保持可理解、可测试。
- 项目可以做独立设计取舍。
